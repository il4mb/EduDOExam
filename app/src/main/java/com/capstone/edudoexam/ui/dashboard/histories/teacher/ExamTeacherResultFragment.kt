...
import com.capstone.edudoexam.databinding.FragmentExamTeacherResultBinding
import com.capstone.edudoexam.models.Answer
import com.capstone.edudoexam.models.AnswerWrapper
...
import com.capstone.edudoexam.ui.dashboard.histories.student.ExamStudentResultFragment.Companion
...
class ExamTeacherResultFragment :
    ...
    private var answers: AnswerWrapper? = null
    ...
    private fun liveCycleObserve() {
        liveData.result.observe(viewLifecycleOwner) {
            lifecycleScope.launch { delay(400); setLoading(false) }
            binding.apply {
    ...
    private fun updateUI(answerWrapper: AnswerWrapper?, questions: MutableList<Question>) {
        val questionsLength = questions.size
        val wrongLength   = answerWrapper?.getTotalWrong(questions) ?: questionsLength
        val correctLength = answerWrapper?.getTotalCorrect(questions) ?: 0
        binding.apply {
        ...
        updateSummaryUI(answerWrapper?.summaryAccumulation())
    }
    ...
    override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
        (parent?.adapter as? UserSpinnerAdapter)?.getItem(position)?.let { user->
            val answersWrapper: MutableList<AnswerWrapper> = liveData.result.value?.answers?: mutableListOf()
    ...
    override fun onNothingSelected(parent: AdapterView<*>?) {
        (parent?.adapter as? UserSpinnerAdapter)?.getItem(0)?.let { user->
            val answers: MutableList<AnswerWrapper> = liveData.result.value?.answers?: mutableListOf()
